...
void cluster_aux(int indice, const std::vector<std::vector<float>> points, std::vector<int>& cluster, std::vector<bool>& processed_points, KdTree* tree, float distanceTol)
{
	processed_points[indice] = true;
	cluster.push_back(indice);

	std::vector<int> nearest_indices = tree->search(points[indice],distanceTol); // Which points
	// are nearby to the point corresponding to that indice within a certain distance tolerance

	for (int id : nearest_indices)
	{
		if (!processed_points[id]) // If some of the nearby points were not included in 
		// a certain cluster, include into this one
		{
			cluster_aux(id,points,cluster,processed_points,tree,distanceTol);
		}
	}
}
...
std::vector<std::vector<int>> euclidean_cluster(const std::vector<std::vector<float>> points, KdTree* tree, float distanceTol)
{
	...
	while (i < points.size())
	{
		...
		cluster_aux(i,points,cluster,processed_points,tree,distanceTol); 
...
int main ()
{
    ...
    for (int i=0; i<points.size(); i++) 
  	...
  	std::vector<std::vector<int>> clusters = euclidean_cluster(points, tree, 3.0);
